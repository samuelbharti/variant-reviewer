# How to cite

Please cite this repository when using it in publications or derivative projects.

Recommended citation (APA):

Bharti, S. (2026). *Variant Reviewer* (version 2.3.1) [Software]. Zenodo. <https://doi.org/10.5281/zenodo.21950635>

BibTeX:

```bibtex
@software{variant_reviewer_2026,
  author = {Bharti, Samuel},
  title = {Variant Reviewer},
  version = {2.3.1},
  doi = {10.5281/zenodo.21950635},
  url = {https://doi.org/10.5281/zenodo.21950635}
}
```

Notes:

- The DOI above (`10.5281/zenodo.21950635`) is the version DOI for 2.3.1. To
  cite the software in general, use the concept DOI `10.5281/zenodo.21934011`,
  which always resolves to the latest version.
- For automated citation support on GitHub, keep a `CITATION.cff` in the
  repository root.
